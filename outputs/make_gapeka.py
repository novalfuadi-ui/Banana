...
def build(scn):
    ...
    for name, km, typ in STATIONS:
        ax_name.text(0.97, km, name, ha="right", va="center", fontsize=10,
    ...
    for name, km, typ in STATIONS:
        ax_km.text(0.5, km, f"KM {km:g}", ha="center", va="center", fontsize=8.5, color="#444")
    ...
    ax_grad.text(0.5, -10, "turun arah hilir\n(maks 0,6%)", ha="center", va="top",
                 fontsize=6.8, style="italic", color="#7a4b00")
    ...
    ax_main.set_xticklabels([hhmm(t) for t in xt], fontsize=7.5, rotation=0)
